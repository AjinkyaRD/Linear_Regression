#House price prediction using linear regression with lasso regularization:
...
def cost(x_train,y_train,w,b,lamda):
    m=y_train.shape[0]
    cost=0
    for i in range(m):
        f=np.dot(x_train[i],w)+b
        cost+=(f-y_train[i])**2
    total_cost=(1/(2*m))*cost + ((lamda/(2*m))*np.sum(np.abs(w)))
    return total_cost

#calculation of derivative term:
def grad(x_train,y_train,w,b,lamda):
    m=y_train.shape[0]
    dw,db=0,0
    for i in range(m):
        f=np.dot(x_train[i],w)+b
        ...
        db+=(f-y_train[i])
    dw=(dw/m) + ((lamda/(2*m))*np.abs(w)/w)
    db=db/m
    return dw,db

#applying gradient_descent algorithm:
def gradient_descent(x_train,y_train,w,b,alfa,lamda):
    co=[]
    iter=[]
    m=y_train.shape[0]
    for i in range(3000):
        co.append(cost(x_train,y_train,w,b,lamda))
        iter.append(i)
        d_w,d_b=grad(x_train,y_train,w,b,lamda)
        w-=(alfa*d_w)
        b-=(alfa*d_b)
    return co,iter,w,b
#input:
w=np.random.rand(6)
b=np.random.rand(1)
p,q,w_last,b_last=gradient_descent(x_train,y_train,w,b,0.001,10)

# plotting of data:
plt.plot(q,p,c='g')
plt.xlabel('No. of Iterations')
plt.ylabel('Cost')
plt.title('Cost Vs Iterations_lasso')
plt.grid()
plt.show()
